		...
		if ( !getMessages()->isError() ) {
			...
			try {
				...
				if( $this->form->pass == $record['password'] ){
					...
					$privilege = getDB()->get("user_has_role","role_idrole",[
						"user_Iduser" => $record['Iduser']
					]);
					
					if ( $privilege == 1) {

                    	$user = new User($this->form->login, $record['Iduser'], 'admin' );

                	} elseif ( $privilege == 2 ) {

                	    $user = new User($this->form->login, $record['Iduser'], 'moderator' );

                	} elseif ( $privilege == 3 ){

                	    $user = new User($this->form->login, $record['Iduser'], 'user' );

                	}else{

						$user = new User($this->form->login, $record['Iduser'], 'banned' );
